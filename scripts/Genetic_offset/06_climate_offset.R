...
faux <- read_csv("data/genomic_data/faux_snp.csv") #pop data
test_snp <- faux %>% dplyr::select(-Site_Name, -Paper_ID, -Latitude, -Longitude, -Elevation, -MAT, -MAP, -CMD,
                                              -PAS, -EXT, -Tave_wt, -Tave_sm, -PPT_wt, -PPT_sm)

## Generate specific dataframes for GF model
env_site <- faux %>% dplyr::select(MAT,MAP,PAS,EXT,CMD,Tave_wt,Tave_sm,PPT_wt,PPT_sm)
...
writeRaster(mask_offset_2012,"data/genomic_data/offset_climate.tif", format="GTiff", overwrite=TRUE)